...
const AdminPage: React.FC<AdminPageProps> = ({
  ...
  const [newBuzzerThresholds, setNewBuzzerThresholds] = useState<number[]>(
    buzzerThresholds.slice()
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "95vh" }}>
                ...
                Object.keys(sensorData[0]).map((keyName, idx) => {
                  if (keyName != "time") {
                    return (
                      <Line
                        dataKey={keyName}
                        stroke={keyToColorMapping[keyName]}
                      />
                    );
                  }
                  return <></>;
            ...
            return (
              <div>
                ...
                <input
                  ...
                  onBlur={() =>
                    setBuzzerThreshold(idx, newBuzzerThresholds[idx])
                  }
                />
              </div>
            );
      ...
      <div style={{ flex: 1 }} className="row">
        <button onClick={() => requestArduinoMode(ArduinoMode.LOG_TOUCH)}>
          tournament mode
        </button>
      </div>
    </div>
